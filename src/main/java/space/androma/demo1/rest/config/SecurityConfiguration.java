...
*/
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable().authorizeRequests()
				...
				/*.antMatchers("/devices/**", "/values/**").hasAnyRole("ENGINEER", "DEVELOPER")
				.antMatchers("/monitor/**").hasAnyRole("VIEWER", "ENGINEER", "ADMIN", "DEVELOPER", "OPERATOR")
				.antMatchers("/admin/**", "/users/**").hasAnyRole("ADMIN", "DEVELOPER")*/
	...
	//Log.info("хуйговноимуровей");
	}

	@Autowired
	public void configureGlobal(AuthenticationManagerBuilder auth) throws Exception {
		auth.inMemoryAuthentication().withUser("rom").password("123").roles("ADMIN");
